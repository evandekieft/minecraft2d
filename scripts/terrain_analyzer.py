...
from visualize_terrain import generate_terrain_map, calculate_terrain_stats
...
class TerrainAnalyzer:
    ...
    def analyze_distribution(self, width=200, height=150, seed=42, iterations=1):
        ...
        result = {
            ...
            'parameters': {'width': width, 'height': height, 'seed': seed, 'iterations': iterations}
        }
    ...
    def _average_stats(self, stats_list):
        ...
        for block_type in all_block_types:
            counts = [stats.get(block_type, {'count': 0, 'percentage': 0.0})['count'] for stats in stats_list]
            percentages = [stats.get(block_type, {'count': 0, 'percentage': 0.0})['percentage'] for stats in stats_list]
    ...
    def compare_to_target(self, stats):
        ...
        print("-" * 60)
        print(f"{'Block Type':>12} | {'Target':>8} | {'Actual':>8} | {'Diff':>8} | {'Status'}")
        print("-" * 60)
        ...
        for block_type, target_pct in target_distribution.items():
            ...
            print(f"{block_type:>12} | {target_pct:>7.1f}% | {actual_pct:>7.1f}% | {diff:>+6.1f}% | {status}")
        ...
        for block_type in stats:
            if block_type not in target_distribution:
                ...
                print(f"{block_type:>12} | {'N/A':>8} | {actual_pct:>7.1f}% | {'N/A':>8} | -")
    ...
    def auto_tune(self, max_iterations=10, target_error=5.0, width=200, height=150, seed=42):
        """
        ...
        """
        print(f"Auto-tuning terrain parameters (max {max_iterations} iterations, target error: {target_error}%)")
        print("=" * 70)
    ...
    def generate_report(self, output_file=None):
        ...
        for layer in self.config.base_layers:
            report.append(f"  {layer.name}: threshold={layer.threshold:.3f}, target={layer.target_percentage:.1f}%")
        
        report.append("\nFeature Rules:")
        for rule in self.config.feature_rules:
            report.append(f"  {rule.name}: chance={rule.spawn_chance:.2f}, target={rule.target_percentage:.1f}%")
        ...
        for block_type, data in sorted(stats.items(), key=lambda x: x[1]['percentage'], reverse=True):
            actual_pct = data['percentage']
            target_pct = target_dist.get(block_type, 0)
            diff = actual_pct - target_pct
            report.append(f"  {block_type}: {actual_pct:.1f}% (target: {target_pct:.1f}%, diff: {diff:+.1f}%)")
    ...
    def _serialize_config(self):
        ...
        return {
            'base_layers': [(layer.name, layer.threshold, layer.target_percentage) for layer in self.config.base_layers],
            'feature_rules': [(rule.name, rule.spawn_chance, rule.target_percentage) for rule in self.config.feature_rules],
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Advanced terrain analysis and tuning tool")
    ...
    parser.add_argument("--iterations", type=int, default=1, help="Number of analysis iterations")
    parser.add_argument("--auto-tune", action="store_true", help="Auto-tune parameters")
    parser.add_argument("--max-tune-iterations", type=int, default=10, help="Max auto-tune iterations")
    parser.add_argument("--target-error", type=float, default=5.0, help="Target error threshold")
