...
class TerrainConfig:
    ...
    def __init__(self):
        ...
        self.feature_rules = [
            FeatureRule("wood", ["grass", "dirt"], 0.75, 0.3, False, 10.0, "Trees on grass/dirt"),
            FeatureRule("lava", ["stone"], 0.80, 0.4, True, 5.0, "Lava in deep areas"),
            FeatureRule("diamond", ["stone"], 0.30, 0.6, True, 1.0, "Rare diamonds deep underground"),
            FeatureRule("coal", ["stone"], 0.15, 0.2, False, 0.0, "Coal in stone areas"),
        ]
        
        # Noise generation parameters
        self.noise_params = {
            'large_scale': {'scale': 0.002, 'octaves': 3, 'persistence': 0.5, 'lacunarity': 2.0},
            'medium_scale': {'scale': 0.01, 'octaves': 4, 'persistence': 0.6, 'lacunarity': 2.0},
            'small_scale': {'scale': 0.05, 'octaves': 2, 'persistence': 0.3, 'lacunarity': 2.0},
            'feature_scale': 0.05,
        ...
        for layer in self.base_layers:
            if layer.name in actual_distribution:
                ...
                if abs(difference) > 2.0:  # Only adjust if difference is significant
                    ...
                    layer.threshold = max(0.1, min(0.9, layer.threshold))  # Clamp to reasonable range
                    
                    print(f"  {layer.name}: {actual_pct:.1f}% -> {target_pct:.1f}% (threshold: {layer.threshold:.3f})")
    ...
    def validate_configuration(self) -> List[str]:
        ...
        for i in range(1, len(self.base_layers)):
            if self.base_layers[i].threshold <= self.base_layers[i-1].threshold:
                issues.append(f"Base layer thresholds must be in ascending order: {self.base_layers[i-1].name} -> {self.base_layers[i].name}")
        ...
        for rule in self.feature_rules:
            for base_name in rule.base_terrain:
                if base_name not in base_terrain_names and base_name != "dirt":  # dirt is a special case
                    issues.append(f"Feature rule '{rule.name}' references unknown base terrain '{base_name}'")
        
        # Check that target percentages are reasonable
        total_base_target = sum(layer.target_percentage for layer in self.base_layers)
        total_feature_target = sum(rule.target_percentage for rule in self.feature_rules)
        
        if total_base_target > 100:
            issues.append(f"Base terrain target percentages sum to {total_base_target}%, which exceeds 100%")
        
        if total_base_target + total_feature_target > 100:
            issues.append(f"Total target percentages sum to {total_base_target + total_feature_target}%, which exceeds 100%")
